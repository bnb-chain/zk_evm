...
pub(crate) fn prove_with_traces_batch<F, C, const D: usize>(
    all_stark: &AllStark<F, D>,
    config: &StarkConfig,
    trace_poly_values: [Vec<PolynomialValues<F>>; NUM_TABLES],
    public_values: PublicValues,
    ...
    C: GenericConfig<D, F = F>,
{
    ...
    let quotient_polys = all_quotient_polys(
        all_stark,
        &trace_commitment,
    ...
    let mut all_fri_instances: Vec<FriInstanceInfo<F, D>> = Vec::new();
...
fn all_quotient_polys<F, C, const D: usize>(
    all_stark: &AllStark<F, D>,
    ...
    C: GenericConfig<D, F = F>,
{
    let res = Vec::new();
    // Arithmetic.
    // let arith_quot = compute_quotient_polys::<F, <F as Packable>::Packing, C, S,
    // D>(     &all_stark.arithmetic_stark,
    //     &trace_commitment,
    //     &auxiliary_commitment,
    //     lookup_challenges,
    //     ctl_data_per_table[*Table::Arithmetic],
